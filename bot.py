...
async def share_bandwidth_for_all_tokens(proxies=None):
    ...
    for index, token in enumerate(tokens):
        ...
        # 添加随机延迟，等待后再执行下一个循环
        delay = random.uniform(5, 10)  # 随机延迟 5 到 10 秒
        logger(f"等待 {delay:.2f} 秒后继续处理下一个账号...", 'info')
...
def register_user():
    try:
        ...
        invite_code =ask_question('请输入您的邀请码（所有账户将使用这个邀请码）: ')#ole68ec966
